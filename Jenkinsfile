pipeline {
  ...
  environment {
    BOT_NAME = 'memory_synthesizer_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-memory_synthesizer"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/memory_synthesizer_bot/bin/memory_synthesizer_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyMemorySynthesizer.Release.create()'"
          $RELEASE_BIN eval 'BotArmyMemorySynthesizer.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyMemorySynthesizer.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyMemorySynthesizer.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_memory_synthesizer/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_memory_synthesizer/releases/start_erl.data)
        fi
